...
void ChaosMod::DrawText(char* text, Vector2 position, Vector2 scale, LinearColor color, bool bCenter, int shadowSize,
...
void ChaosMod::ToggleModStatus()
{
	...
	if (bEnabled)
	{
		...
		ShowNotification2("~COLOR_GREEN~Chaos Mod Enabled", effectsNumStr.c_str(), 3500, "scoretimer_textures",
						  "scoretimer_generic_tick", LinearColor(77, 170, 104, 255));
	}
	else
	{
		...
		ShowNotification2("~COLOR_PLAYER_STATUS_NEGATIVE~Chaos Mod Disabled", "", 3500, "scoretimer_textures",
						  "scoretimer_generic_cross", LinearColor(204, 0, 0, 255));
...
void ChaosMod::StartNodeProcess()
{
	...
	auto processCreationResult = CreateProcessW(NULL, exePath_c, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, chaosDir_c,
												&NodeStartupInfo, &NodeProcessInformation);
...
void ChaosMod::DrawUI()
{
	...
	float effectTimeoutValue = float(
			float(GetTickCount() - timeoutStartTime) / float(timeoutEndTime - timeoutStartTime));
	
	if (effectTimeoutValue < 0.0f)
	{
		effectTimeoutValue = 0.0f;
	}
	else if (effectTimeoutValue > 1.0f)
	{
		effectTimeoutValue = 1.0f;
	}
	...
	GRAPHICS::DRAW_RECT(effectTimeoutValue / 2.0f, ProgressBarHeight / 2.0f, effectTimeoutValue, ProgressBarHeight,
...
void ChaosMod::DrawEffectInUI(Effect* effect, int32_t index)
{
	...
	{
		float progressBarValue = 1.0f - (((float) GetTickCount() - (float) effect->ActivationTime) /
										 ((float) effect->DeactivationTime - (float) effect->ActivationTime));
		float progressBarWidth = (400.0f / 1920.0f);
		float progressBarX = X - (((1.0f - (progressBarValue)) / 2.0f) * progressBarWidth);
		GRAPHICS::DRAW_RECT(progressBarX, Y, (progressBarWidth) * progressBarValue, 40.0f / 1080.0f, 0, 0, 0, 185, 0,
							0);
	}
...
void ChaosMod::InitEffects()
{
	AllEffects = {new EffectSpawnSoldier(), new EffectSpawnLenny(), new EffectSpawnDrunkardJon(),
				  ...
				  new EffectThunderstorm(), new EffectGivePlayerMoney(), new EffectBankruptcy(),
				  new EffectSpawnVampire(), new EffectGiveRifle(), new EffectGiveRevolver(),
				  new EffectRemoveAllWeapons(), new EffectNoAmmo(), new EffectDropWeapon(), new EffectHealPlayer(),
				  new EffectAlmostDead(), new EffectRestoreStamina(), new EffectFlipAllVehs(), new EffectGiantPeds(),
				  new EffectAllPedsWannaKillPlayer(), new EffectSpawnMiniDonkey(), new EffectSpawnGiantDonkey(),
				  new EffectRagdoll(), new EffectRagdollEveryone(), new EffectSpawnGiantCop(), new EffectLaunchPedsUp(),
				  new EffectInvertedGravity(), new EffectDoomsday(), new EffectGiveRandomWeapon(), new EffectSetDrunk(),
				  new EffectSpawnAngryDwarf(), new EffectSpawnAngrySkeleton(), new EffectSpawnCompanionBertram(),
				  new EffectPlayIntro(), new EffectSetRandomWeather(), new EffectSetSunnyWeather(),
				  new EffectSetRainyWeather(), new EffectSetRapidWeather(), new EffectSetTimeMorning(),
				  new EffectSetTimeNight(), new EffectSetRandomTime(), new EffectClearPursuit(),
				  new EffectIncreaseBounty(), new EffectSpawnFrozenCouple(), new EffectEarthquake(),
				  new EffectRemoveCurrentVehicle(), new EffectTrainsawLaser(), new EffectMinecartRain(),
				  new EffectGiveLasso(), new EffectFullAcceleration(), new EffectEveryoneIsInvincible(),
				  new EffectEveryoneExitsVehs(), new EffectSetPlayerIntoRandomVeh(), new Effect120FOV(),
				  new EffectSpawnRobot(), new EffectSpawnHotAirBalloon(), new EffectSpawnLassoGuy(),
				  new EffectIgnitePlayersWagon(), new EffectIgniteNearbyWagons(), new EffectIgnitePlayer(),
				  new EffectIgniteNearbyPeds(), new EffectLightningOnce(), new EffectKickflip(), new EffectSpawnCanoe(),
				  new EffectHorsesRain(), new EffectLightningEnemy(), new EffectLightningWeapons(),
				  new EffectSkyrimIntro(), new EffectCowSkin(), new EffectDwarfSkin(), new EffectPigSkin(),
				  new EffectRatSkin(), new EffectTurtleSkin(), new EffectSpawnParrotCompanion(), new EffectHonorGood(),
				  new EffectHonorBad(), new EffectHonorReset(), new EffectSpawnShireHorse(), new EffectAltTab(),
				  new EffectPlayerSleep(), new EffectPlayerIsMinion(), new EffectTeleportToWaypoint(),
				  new EffectRemoveWeaponFromEveryone(), new EffectUndeadNightmare(), new EffectBloodTrails(),
				  new EffectDetachWheels(), new EffectExplosiveWeapons(), new EffectSetFoggyWeather(), new EffectSetFroggyWeather(),
				  new EffectGhostTown(), new EffectSpawnDogCompanion(), new EffectSpawnCatCompanion(),
				  new EffectSetPedsIntoPlayerVehicle(), new EffectSetRandomWalkStyle(), new EffectTeleportWeapons(),
				  new EffectGiveSniperRifle(), new EffectGiveDynamite(), new EffectThrowingKnives(),
				  new EffectSpawnBearCompanion(), new EffectTeleportFewMeters(), new EffectBlackingOut(),
				  new EffectRandomClothes(), new EffectSpawnAngryCorpse(), new EffectFastPlayersWagon(),
				  new EffectSpawnAngryCaveman(), new EffectSpawnAngryTwin(), new EffectMostWanted(),
				  new EffectSpawnUFO(), new EffectGravityField(), new EffectPigWeapons(), new EffectSpawnAngryCowboy(),
				  new EffectSpawnUndeadBoss(), new EffectSpawnGrieferMicah(), new EffectSetWinterOutfit(),
				  new EffectGamespeedx02(), new EffectGamespeedx05(), new EffectSuperJump(),
				  new EffectDisableLeftRight(), new EffectDisableForwardBackward(), new EffectDisableAllMovements(),
				  new EffectDisableSprintJump(), new EffectDisableAttackButton(), new EffectDisableAiming(),
				  new EffectRandomWheelsDetaching(), new EffectRainingPigs(), new EffectRandomHonor(),
				  ...
				  new EffectSetRandomVelocity(), new EffectExplodeNearbyPeds(), new EffectNearbyPedIsCompanion(),
				  new EffectEveryoneRagdollsWhenShot(), new EffectNearbyPedIsEnemy(), new EffectExplosiveCombat(),
				  new EffectGiveEveryoneRifle(), new EffectTimelapse(), new EffectNoHUD(), new EffectFirstPerson(),
				  new EffectTopDownCamera(), new EffectAgitateHorse(), new EffectODriscolls(),
				  new EffectInsaneGravity(), new EffectUpsideDownCamera(), new EffectOneHitKO(),
				  ...
				  new EffectChaosRain(), new EffectSpawnCompanionDutch(), new EffectSpawnKillerBunnyHorde()};
...
void ChaosMod::ShowNotification(const char* title, const char* subtitle, const char* iconDict, const char* iconName,
								int32_t durationMs, const char* iconColor)
{
	if (!notificationDuration)
	{
...
void ChaosMod::ShowNotification2(const char* title, const char* subtitle, uint32_t durationMs, const char* iconDict,
...
void ChaosMod::RenderNotification2()
{
	...
	{
		opacity = float(timeNow - notification2Data.showTime) /
				  float(notification2Data.showTime + fadeTime - notification2Data.showTime);
	}
	...
	GRAPHICS::DRAW_SPRITE((char*) "feeds", (char*) "help_text_bg", 0.5f, 0.5f + yOffset, 512.0f / 1920.0f,
						  128.0f / 1080.0f, 0.0f, 0, 0, 0, alpha, false);
	
	/** Icon color */
	LinearColor iconC = notification2Data.iconColor;
	iconC.A = uint8_t(float(iconC.A) * opacity);
	
	GRAPHICS::DRAW_SPRITE((char*) notification2Data.icon.dict, (char*) notification2Data.icon.name, 0.41f,
						  0.5f + yOffset, 0.05f, 96.0f / 1080.0f, 0.0f, iconC.R, iconC.G, iconC.B, iconC.A, false);
	...
	notification2Data.__titleVarStr = GAMEPLAY::CREATE_STRING(10, (char*) "LITERAL_STRING",
															  (char*) notification2Data.title.c_str());
	notification2Data.__titleVarStr = invoke<const char*>(0xFA925AC00EB830B9, 42, (char*) "COLOR_STRING", 0,
														  notification2Data.__titleVarStr);
	...
	if (notification2Data.bSubtitleExists)
	{
		...
		auto subtitle = GAMEPLAY::CREATE_STRING(10, (char*) "LITERAL_STRING",
												(char*) notification2Data.subtitle.c_str());
...
std::vector<Effect*> ChaosMod::GenerateEffectsWithChances(uint32_t maxEffects)
{
	...
	for (uint32_t i = 0; i < config.effects.size(); i++)
	{
		 ...
		 */
		if (!configEffect.bEnabled || oldEffectIDs.contains(configEffect.id) ||
			(prevActivatedEffect && prevActivatedEffect->ID == configEffect.id))
		{
			continue;
		}
	...
	for (uint32_t i = 0; i < maxEffects; i++)
	{
		...
		effectsIndices.erase(effectsIndices.begin() + effectIndexStart,
							 effectsIndices.begin() + (effectIndexStart + configEffect.chance));
	}
